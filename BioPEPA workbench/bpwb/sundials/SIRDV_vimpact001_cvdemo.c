/*
 *  SIRDV_vimpact001_cvdemo.c
 *
 *
 *  CVODE ODE solver for the vector field named: SIRDV_vimpact001
 *
...
int main (int argc, char *argv[])
{
    ...
    const int N_ = 6;
    const int P_ = 5;
    const realtype def_p_[5] = {
        RCONST(1.0),
        RCONST(1.0),
        RCONST(6.6000000000000003e-01),
        RCONST(1.9600000000000001e-01),
        RCONST(4.0000000000000001e-03)
    };
    const realtype placeholder_ = def_p_[0];
    const realtype vaccinerate_ = def_p_[1];
    const realtype percentage_ = def_p_[2];
    const realtype gamma1_    = def_p_[3];
    const realtype alpha_     = def_p_[4];
    realtype def_y_[6] = {RCONST(59641488.0), RCONST(2.0), RCONST(0.0), RCONST(0.0), RCONST(0.0), RCONST(2.0)};
    realtype y_[6];
    realtype p_[5];
    realtype solver_param_[3] = {RCONST(1.0e-6), RCONST(0.0), RCONST(10.0)};
    char *varnames_[6] = {"S_", "I_", "R_", "D_", "V_", "CUMI_"};
    char *parnames_[5] = {"placeholder_", "vaccinerate_", "percentage_", "gamma1_", "alpha_"};
    ...
    flag = CVodeMalloc(cvode_mem, SIRDV_vimpact001_vf, t, y0_, CV_SS, solver_param_[1], &(solver_param_[0]));
    flag = CVodeSetFdata(cvode_mem, &(p_[0]));
    flag = CVDense(cvode_mem, N_);
    flag = CVDenseSetJacFn(cvode_mem, SIRDV_vimpact001_jac, &(p_[0]));
