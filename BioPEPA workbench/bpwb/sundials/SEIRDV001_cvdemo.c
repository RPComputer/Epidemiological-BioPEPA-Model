 ...
 *
 *
 *  CVODE ODE solver for the vector field named: SEIRDV001
 *
...
int main (int argc, char *argv[])
{
    ...
    const realtype delta_     = def_p_[2];
    const realtype gamma1_    = def_p_[3];
    const realtype alpha_     = def_p_[4];
    realtype def_y_[7] = {RCONST(60000000.0), RCONST(12.0), RCONST(0.0), RCONST(0.0), RCONST(0.0), RCONST(0.0), RCONST(0.0)};
    realtype y_[7];
    realtype p_[5];
    realtype solver_param_[3] = {RCONST(1.0e-6), RCONST(0.0), RCONST(10.0)};
    char *varnames_[7] = {"S_", "E_", "I_", "R_", "D_", "V_", "CUMI_"};
    char *parnames_[5] = {"placeholder_", "vaccinerate_", "delta_", "gamma1_", "alpha_"};
    ...
    flag = CVodeMalloc(cvode_mem, SEIRDV001_vf, t, y0_, CV_SS, solver_param_[1], &(solver_param_[0]));
    flag = CVodeSetFdata(cvode_mem, &(p_[0]));
    flag = CVDense(cvode_mem, N_);
    flag = CVDenseSetJacFn(cvode_mem, SEIRDV001_jac, &(p_[0]));
