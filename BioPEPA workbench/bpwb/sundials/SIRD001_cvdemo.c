 ...
 *
 *
 *  CVODE ODE solver for the vector field named: SIRD001
 *
...
int main (int argc, char *argv[])
{
    ...
    const realtype gamma1_    = def_p_[1];
    const realtype alpha_     = def_p_[2];
    realtype def_y_[4] = {RCONST(60000000.0), RCONST(2.0), RCONST(0.0), RCONST(0.0)};
    realtype y_[4];
    realtype p_[3];
    realtype solver_param_[3] = {RCONST(1.0e-6), RCONST(0.0), RCONST(10.0)};
    char *varnames_[4] = {"S_", "I_", "R_", "D_"};
    char *parnames_[3] = {"placeholder_", "gamma1_", "alpha_"};
    ...
    flag = CVodeMalloc(cvode_mem, SIRD001_vf, t, y0_, CV_SS, solver_param_[1], &(solver_param_[0]));
    flag = CVodeSetFdata(cvode_mem, &(p_[0]));
    flag = CVDense(cvode_mem, N_);
    flag = CVDenseSetJacFn(cvode_mem, SIRD001_jac, &(p_[0]));
