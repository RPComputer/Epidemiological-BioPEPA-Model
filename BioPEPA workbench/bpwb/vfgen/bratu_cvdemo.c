 ...
 *
 *
 *  CVODE ODE solver for the vector field named: bratu
 *
...
int main (int argc, char *argv[])
{
    ...
    const realtype a          = def_p_[0];
    realtype def_y_[2] = {RCONST(1.0), RCONST(0.0)};
    realtype y_[2];
    realtype p_[1];
    realtype solver_param_[3] = {RCONST(1.0e-6), RCONST(0.0), RCONST(10.0)};
    char *varnames_[2] = {"x", "y"};
    char *parnames_[1] = {"a"};
    ...
    flag = CVodeMalloc(cvode_mem, bratu_vf, t, y0_, CV_SS, solver_param_[1], &(solver_param_[0]));
    flag = CVodeSetFdata(cvode_mem, &(p_[0]));
    flag = CVDense(cvode_mem, N_);
    flag = CVDenseSetJacFn(cvode_mem, bratu_jac, &(p_[0]));
    ...
    realtype funcval[1];
    bratu_func(t, y0_, funcval, (void *) p_);
    printf(" %.8e",funcval[0]);
    printf("\n");
