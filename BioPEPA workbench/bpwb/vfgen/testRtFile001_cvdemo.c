/*
 *  testRtFile001_cvdemo.c
 *
 *
 *  CVODE ODE solver for the vector field named: testRtFile001
 *
...
int main (int argc, char *argv[])
{
    ...
    const realtype C_Rt       = def_p_[3];
    realtype def_y_[4] = {RCONST(1000000.0), RCONST(5.0), RCONST(0.0), RCONST(0.0)};
    realtype y_[4];
    realtype p_[4];
    realtype solver_param_[3] = {RCONST(1.0e-6), RCONST(0.0), RCONST(10.0)};
    char *varnames_[4] = {"S_", "I_", "R_", "D_"};
    char *parnames_[4] = {"k1_", "k2_", "k3_", "C_Rt"};
    ...
    flag = CVodeMalloc(cvode_mem, testRtFile001_vf, t, y0_, CV_SS, solver_param_[1], &(solver_param_[0]));
    flag = CVodeSetFdata(cvode_mem, &(p_[0]));
    flag = CVDense(cvode_mem, N_);
    flag = CVDenseSetJacFn(cvode_mem, testRtFile001_jac, &(p_[0]));
